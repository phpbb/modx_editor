...
void ModXWriter::write(QIODevice *device)
{
	...
	writeProcessingInstruction("xml-stylesheet", "type=\"text/xsl\" href=\"1.2.1/modx.prosilver.en.xsl\"");
	writeComment(
	...
	for (QMap<QString, QString>::ConstIterator i = m_data->authorNotes.constBegin(); i != m_data->authorNotes.constEnd(); ++i)
	{
		...
		writeStartElement("installation");
			writeStartElement("level");
				writeCharacters(m_data->installLevel);
			writeEndElement();
		...
		writeStartElement("meta");
			writeCharacters("Generated by ModX editor v" + QCoreApplication::applicationVersion());
		writeEndElement();
